...
Chart.defaults.global.defaultFontColor = '#858796';
  ...
  if (tv_count == 0 || JSON.stringify(tvData[user_Id]) === '{}' ) {
    ...
    document.getElementById('TV').innerHTML += 
    ' <p style="display:flex; align-items:center; margin-bottom: 0; margin-left:0.5em"> 하루 평균 TV 시청 횟수가 5회를 넘었어요! TV 시청 횟수를 줄이고 다른 활동을 더 해보시는 건 어떠세요? </p>'
  } else {
      document.getElementById('TV').innerHTML = 
      '하루에 TV를 평균 ' + tv_count + '회 보시네요!' + ' <br /> '
      + Most_Watch + '시에 가장 TV를 많이 보시는군요? 그 시간대에는' + ' <br /> '
      + tv_program[parseInt(Most_Watch)] + ' 방영되고 있어요!'
  }
  ...
  if (snack_count == 0 || JSON.stringify(snackData[user_Id]) === '{}' ) {
    ...
    snack_container.innerHTML += 
    ' <p style="display:flex; align-items:center; margin-bottom: 0; margin-left:0.5em"> 하루 평균 간식 섭취 횟수가 5회를 넘었어요! ' + 
      '하루에 3번으로 횟수를 줄여보시는 건 어떨까요? 달걀, 콩, 견과류에 단백질과 지방이 많아 포만감이 오래간대요!</p>'
  } else {
    snack_container.innerHTML = 
      '하루에 간식을 평균 ' + snack_count + '회 정도 드시고 있어요.'
    snack_container.innerHTML += 
    ' 간식도 규칙적으로 섭취하는 것이 좋다고 해요!'
  }
  ...
  if (JSON.stringify(sleepData[user_Id]) === '{}'){
    ...
    sleep_container.innerHTML = '수면 데이터가 존재하지 않습니다!'
  } else if (user_std < 3) {
    sleep_container.innerHTML = '규칙적인 수면을 하시고 있어요!'
    if (user_std < 1) { user_std = 1}
    sleep_score = (5 * (11 - user_std)).toFixed(0)
  } else {
    sleep_container.innerHTML = '불규칙적인 수면을 하시고 있어요! 규칙적인 수면은 건강에 무척 중요하답니다.'
    sleep_score = (5 * (11 - user_std)).toFixed(0)
  }
  ...
  if (JSON.stringify(exData[user_Id]) === '{}'){
    ex_data_exist = false
    ex_container.innerHTML = '운동 데이터가 존재하지 않습니다!'
  } else if (user_Wk < 1) {
    ex_data_exist = false
    ex_container.innerHTML = '7일 이상 이용하시면 운동 분석 데이터를 제공해드려요!'
  } else if (Ex_Count < 3){
    ex_container.innerHTML = '주 ' + Ex_Count + '회 운동하고 계세요!' + '<br />' +
    ...
    ex_container.innerHTML = '매일 평균 ' + Daily_Ex_Count + '회 운동하고 계세요!' + '<br />' +
    "꾸준히 노력하고 계시네요. 멋있어요!"
    ex_score = 50
  } else {
    ex_container.innerHTML = '일주일에 평균 ' + Ex_Count + '회 운동하고 계세요!' + '<br />' +
    "꾸준히 노력하고 계시네요. 멋있어요!"
    ex_score = 50
  }


  let total_container = document.getElementById('total')
  ...
  if (sleep_data_exist != true || ex_data_exist != true) {
    ...
    total_container.innerHTML = '건강 점수는 ' + total_score + '점입니다!'
  }
