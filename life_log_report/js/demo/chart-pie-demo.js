...
Chart.defaults.global.defaultFontColor = '#858796';
  ...
  function p_tag(string) {
    let tag = '<p style="display:flex; align-items:center; margin-bottom: 0; margin-left:1em">'
    let tag_end = '</p>'
  ...
  tv_program = ['&lt;헬로 트로트&gt; 재방송이', '월~목요일에 &lt;나이트라인&gt;이', '&lt;헬로 트로트&gt; 재방송이', '&lt;골프왕&gt; 재방송이', '&lt;골프왕&gt; 재방송이', '&lt;진리식당&gt; 재방송이',
  '&lt;건강한 집&gt; 재방송이', '&lt;굿모닝 MBN&gt;이', '평일에 &lt;아침마당&gt;이', '평일에 &lt;신통방통&gt;이', '평일에 &lt;SBS 10 뉴스&gt;가', '&lt;코로나19 통합 뉴스룸&gt;이',
  '일요일에 &lt;전국노래자랑&gt;이', '월~목요일에 &lt;일단 해봐요 생방송 오후 1시&gt;가', '평일에 &lt;사건파일 24&gt;가', '토요일에 &lt;쇼! 음악중심&gt;이', '일요일에 &lt;SBS 인기가요&gt;', '&lt;동물의 왕국&gt;이',
  '평일에 &lt;6시 내고향&gt;이', '화요일에 &lt;이웃집 찰스&gt;가', '&lt;SBS 8 뉴스&gt;가', '화요일에 &lt;헬로트로트&gt;가', '월요일에 &lt;가요무대&gt;가', '월~목요일에 &lt;더 라이브&gt;가']  
  ...
  if (tv_count == 0 || JSON.stringify(tvData[user_Id]) === '{}' ) {
    ...
    src.appendChild(img);
    document.getElementById('TV').innerHTML += 
      p_tag('TV를 자주 보시는 분께는 순이가 시간대별 프로그램을 추천해드리고 있답니다!')
  } else if (tv_count > 5) {
    tv_danger = true
    ...
    document.getElementById('TV').innerHTML += 
    p_tag('하루 평균 TV 시청 횟수가 5회를 넘었어요! TV 시청 횟수를 줄이고 다른 활동을 더 해보시는 건 어떠세요?')
  } else {
    document.getElementById('TV').innerHTML = "" 
    var img = document.createElement("img");
    img.style.objectFit = "contain"
    ...
    tv_rec_string = '하루에 TV를 ' + tv_count + '회 보시네요! <br /> ' +
    + Most_Watch + '시에 가장 TV를 많이 보시는군요? <br /> 그 시간대에는 ' +  tv_program[parseInt(Most_Watch)] + ' 방영되고 있어요!'
  ...
  if (snack_count == 0 || JSON.stringify(snackData[user_Id]) === '{}' ) {
    ...
    snack_container.innerHTML += 
    p_tag('하루 간식 섭취 횟수가 5회를 넘었어요! 3번으로 줄여보시는 건 어떨까요? <br /> 달걀, 콩, 견과류에 단백질과 지방이 많아 포만감이 오래간대요!')
  } else {
    var img = document.createElement("img");
    img.style.objectFit = "contain"
    ...
    snack_container.innerHTML += p_tag('하루에 간식을 평균 ' + snack_count + '회 정도 드시고 있어요. 간식도 규칙적으로 섭취하는 것이 좋다고 해요!')
  }
  ...
  if (JSON.stringify(sleepData[user_Id]) === '{}'){
    ...
    if (user_std < 1) { user_std = 1}
    sleep_score = (5 * (11 - user_std)).toFixed(0)
  } else {
    sleep_container.innerHTML += p_tag('불규칙적인 수면을 하시고 있어요! 규칙적인 수면은 건강에 무척 중요하답니다.')
    sleep_score = (5 * (11 - user_std)).toFixed(0)
  }
  ...
  if (JSON.stringify(exData[user_Id]) === '{}'){
    ...
    ex_data_exist = false
    ex_container.innerHTML += p_tag('7일 이상 이용하시면 운동 분석 데이터를 제공해드려요!')
  } else if (Ex_Count < 3){
    ex_container.innerHTML += p_tag('주 ' + Ex_Count + '회 운동하고 계세요!' + '<br />' +
    ...
    ex_container.innerHTML += p_tag('매일 평균 ' + Daily_Ex_Count + '회 운동하고 계세요!' + '<br />' +
    "꾸준히 노력하고 계시네요. 멋있어요!")
    ex_score = 50
  } else {
    ex_container.innerHTML += p_tag('일주일에 평균 ' + Ex_Count + '회 운동하고 계세요!' + '<br />' +
    "꾸준히 노력하고 계시네요. 멋있어요!")
    ex_score = 50
  }
  ...
  if (sleep_data_exist != true || ex_data_exist != true) {
    total_container.innerHTML = '수면 데이터와 운동 데이터가 모두 있어야 건강 점수를 계산할 수 있어요!'
  } else {
    total_score = sleep_score + ex_score
    if (tv_danger) { total_score -= 10 }
    if (snack_danger) { total_score -= 10}
    ...
    if (total_score < 40) { score.style.color = "red" }
    else if (total_score < 80 ) { score.style.color = "yellow"}
    else { score.style.color = "green"
    score.innerHTML += '!'}
